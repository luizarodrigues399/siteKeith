...
if ( ! class_exists( 'SPLC_FREE_Fields' ) ) {
	 ...
	 * @version 1.0.0
	 */
	abstract class SPLC_FREE_Fields extends SPLC_FREE_Abstract {
		/**
		 * Field
		 *
		 * @var array
		...
		public function field_before() {
			return ( ! empty( $this->field['before'] ) ) ? '<div class="splogocarousel-before-text">' . $this->field['before'] . '</div>' : '';
		}
		...
		public function field_after() {
			...
			$output  = ( ! empty( $this->field['after'] ) ) ? '<div class="splogocarousel-after-text">' . $this->field['after'] . '</div>' : '';
			$output .= ( ! empty( $this->field['desc'] ) ) ? '<div class="clear"></div><div class="splogocarousel-desc-text">' . $this->field['desc'] . '</div>' : '';
			$output .= ( ! empty( $this->field['help'] ) ) ? '<div class="splogocarousel-help"><span class="splogocarousel-help-text">' . $this->field['help'] . '</span><i class="fa fa-question-circle"></i></div>' : '';
			$error   = '';
			if ( ! empty( $this->field['_error'] ) ) {
				if ( is_array( $this->field['_error'] ) ) {
					foreach ( $this->field['_error'] as $key => $value ) {
						$error .= $value;
					}
				} else {
					$error .= $this->field['_error'];
				}
			}
