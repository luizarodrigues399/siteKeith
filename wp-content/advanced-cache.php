...
// WP-Optimize advanced-cache.php (written by version: 4.3.0) (homeurl: http://localhost:41062/keith/) (abspath: /opt/lampp/htdocs/keith/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/opt/lampp/htdocs/keith/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://localhost:41062/keith/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/opt/lampp/htdocs/keith/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-localhost-port41062.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-localhost-port41062.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-localhost-port41062.php');
}
